...
public class GameServerServlet extends HttpServlet {
	...
	public String createGameJSON(Player pl, int gameID) {
		...
		if (g != null) {
			...
			JsonObject value = factory.createObjectBuilder().add("gameID", gameID).add("gameType", g.getType())
					.add("players", players).add("status", g.getStatus()).add("info", g.gameInfo())
					.add("requestedby", pl.getUser().getName()).add("yourturn", g.isPlayersTurn(pl))
					.add("board", g.getBoard()).build();
	...
	public String getUserDataJSON(User u) {
		...
		for (Player p : u.getActiveParticipations()) {
			JsonObjectBuilder game = factory.createObjectBuilder().add("id", p.getGame().getGameID()).add("type",
					p.getGame().getType());
			if (p.getGame().isStarted()) {
				game.add("players",
						p.getGame().getPlayers().stream().map(Player::getName).reduce((x, y) -> x + " vs " + y).get());
			} else {
